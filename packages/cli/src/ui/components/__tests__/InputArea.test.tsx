...
vi.mock("../../contexts/ChatContext.js", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("../../contexts/ChatContext.js")>();
  return {
    ...actual,
    useChatState: () => ({
      messages: [],
...
describe("InputArea", () => {
  ...
  describe("static rendering", () => {
    it("renders input prompt", () => {
      const { lastFrame } = render(
        <InputArea onExit={mockOnExit} width={80} />,
      );
  ...
  describe("slash commands", () => {
    ...
    it("sends unknown commands as messages (allows file paths)", async () => {
      const { stdin } = render(<InputArea onExit={mockOnExit} width={80} />);

      // /unknown is not a known command, so it's sent as a message
      // This allows file paths like /Users/... to be sent as messages
      stdin.write("/unknown");
      await delay(50);
      stdin.write("\r");
      await delay(50);

      expect(mockSendMessage).toHaveBeenCalledWith("/unknown");
  ...
  describe("long text handling", () => {
    ...
    it("submits long text correctly", async () => {
      ...
      const longText =
        "This is a very long message that exceeds typical line length";
      stdin.write(longText);
      await delay(50);
      stdin.write("\r");
      await delay(50);
